...
import { getAllInvitations, acceptAnInvitation, deleteInvitationById } from "../../store/invitations"
import {getAllUserWorkspaces} from "../../store/workspace"

const FindWorkspace = ({closeModal}) => {
            ...
            {Object.values(invitations).map((invitation, i) => (
                <div className="invitation" key={i}>
                    <div className="left-side-invitation">
                        ...
                        <img className="invitation-image" src={invitation.workspace_info.workspace_image.url} alt="workspace" />
                        </div>
                        <div className="invitation-name">{invitation.workspace_info.name}</div>
                    </div>
                    <div>
                        <button className="decline-invite-btn" onClick={() => handleDeclineInv(invitation.id)}>Decline</button>
                        <button className="accept-invite-btn" onClick={() => handleAcceptInv(invitation.id)}>Join</button>
                    </div>
                </div>
            ))}
            {Object.values(invitations).length === 0 && <div className="no-invitations">No pending invitations</div>}
      </div>
    </div>
    );
}
