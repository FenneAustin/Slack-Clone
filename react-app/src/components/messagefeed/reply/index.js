...
function Comment({ comment, sessionUserId }) {
  ...
  useEffect(() => {
    const tempArr = [];
    if (
      newComment.split(" ").length === newComment.length + 1 ||
      newComment.length < 0
    ) {
    ...
    return (
      <div className="comment">
        ...
        <div className="comment-body edit-area">
          ...
          ></textarea>
          <button onClick={saveComment} className="publish-btn">
            Save
          </button>
          <button onClick={destroyComment} className="cancel-btn-updatepage">
            Delete
          </button>
        </div>
      </div>
